...
``` {r get IDs, include=F}
# i figured out that newid is just uncchdid with a 400 before it, so instead of making a "converter" out a dataset, I will just subtract the 400 part
# id.convert <- analyzewealthpc %>% filter(site==4) %>% select(newid, uncchdid) %>% filter(!is.na(uncchdid))
...
- For where there was only one value per ID in the newagecat2 bin we used that value and that age. For where there were multiple values, we used the average of BMI at the average age. <p><mark>This approach assumes linear change in BMI during that time period</mark></p>
...
# df.c is the df that includes changes in bmi by years between the bins
# columns - newid, newagecat2, avg.bmi, and avg.age at bmi
# filling in the survey year for those who are missing it
...
- z-scores were made for education. this variable is called gradecom.z
  ...
  - If in 2005, 2009, or 2018 gradcom is missing (gradecom I got from the gradecom05, etc.), I am applying the number from maxgrade
...
pregs2018 <- read_dta('/Users/rvolosh/Documents/Papers/Cebu incident obesity/preg files/pregsonly0218.dta') %>% mutate(
  ...
  mutate(
    # num pregs per year
    # had issues, so testing w surv.diff = survey - lag(survey)
    # turns out lag not working; switched to Hmisc:Lag(shift=1)
...
Some people who were unemployed (those who had been employed before and those who had never worked before) still had a source of income. I want to see if treating them as employed would change the results, because I don't kjnow what to do with them exactly.

Variable of interest names:
- mainpos = position primarily working in in main job
  - weirdly deinfed. For example, sitting most of the time and moving around most of the time - that makes absolutely NO sense.
  - I am putting sitting and squatting in together into one variable
  - I am puting standing and standing + bending over together as well
...
- mainexe, secexer = does main job require physical exertion? If yes, what kind
  ...
  - dividing into:
    ...
    - light exertion (e.g. sweeping, carrying a child) - 2, 3, 6, 9, 11, 12 (what is a kolador?), 13, 15, 16, 17, 18, 19, 20, 21, 22, 25, 29
...
# for some reason, educat is not appearing as a categorical variables, so I am going to factor it, to make sure it is one henceforth
df7$educat <- factor(df7$educat, levels = c(1, 2, 3, 4))

```

There are some people for whom it says they are not working, but there is a non-zero daily income, so I am assuming that is based on their salary pre-quitting work for the ones with -6 (not working, but has worked before), and for those who have 0 for working, I am not sure. I will do analysis with the ones with 0 or -6 for work putting them in with the earners and also putting them in with the jobless. Then, I will see if it changes my results at all.

Seems like there are no deadbeats...
...
do it as it makes the most sense to you - say you are not exerting yourself if you are not working and then do the other way as a sensitivity analysis
...
##### Write out a file for use in epi
```{r, eval=TRUE, include=TRUE}

save(df7, file = '/Users/rvolosh/Documents/Papers/Cebu Epigenetics/codes/df7.RData')
...
# quantile regression doesn't work for lrtest, so I will just do regular models, and that will be like 0.5 tau, then 0.25, and 0.75
...
Based on the results above, I should interact asset index with the number of pregnancies completed between the surveys in all surveys. Thought it is not the focus of my paper, it represents a real biological variable - maternal depletion vs. weight gain associated with pregnancy in settings of abundance.
...
# women
# glm(pres05 ~ lag.pc + lag.urbindex,
#     data = df8[df8$survey==1986 & df8$sex==0,],
